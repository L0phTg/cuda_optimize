...
/*
 * vectorized, per thread copy 4 element once
 * NOTICE: element must be consistent
 */
template<
    ...
    >
__global__ void gpu_matrix_mult_vectorized(
                int* __restrict__ A,
                int* __restrict__ B,
                int* __restrict__ C,
                int M, int N, int K)
{
    ...
    const int A_TILE_ROW_START=4 * tid /BLOCK_SIZE_K;
    const int B_TILE_ROW_START=4 * tid /BLOCK_SIZE_N;

    const int A_TILE_COL=(4 * tid)%BLOCK_SIZE_K;
    const int B_TILE_COL=(4 * tid)%BLOCK_SIZE_N;

    // row stride that thread uses to load multiple rows of a tile
    const int A_TILE_ROW_STRIDE=4 * THREAD_NUM_PER_BLOCK/BLOCK_SIZE_K;
    const int B_TILE_ROW_STRIDE=4 * THREAD_NUM_PER_BLOCK/BLOCK_SIZE_N;
    ...
    {
        // load A from global memory to shared memory, one thread copy 4 element
        #pragma unroll
        for(int i=0;i<BLOCK_SIZE_M;i+=A_TILE_ROW_STRIDE){
            FETCH_INT4(tile_A[i+A_TILE_ROW_START][A_TILE_COL]) = FETCH_INT4(A[OFFSET(
        ...
        for(int i=0;i<BLOCK_SIZE_K;i+=B_TILE_ROW_STRIDE){
            FETCH_INT4(tile_B[i+B_TILE_ROW_START][B_TILE_COL]) = FETCH_INT4(B[OFFSET(
        ...
        for (int k_idx = 0; k_idx < BLOCK_SIZE_K; k_idx++) {
            ...
            for(int thread_x = 0; thread_x < THREAD_SIZE_X; thread_x+=4) {
                FETCH_INT4(frag_b[thread_x]) = FETCH_INT4(tile_B[k_idx][THREAD_SIZE_X*tx + thread_x]);
            }
 ...
 * NOTICE: element must be consistent
 *
 * transpose tile_A for: vectorized frag_a
 */
template<
    const int BLOCK_SIZE_M,
    const int BLOCK_SIZE_N,
    ...
    const int THREAD_SIZE_X
    >
__global__ void gpu_matrix_mult_vectorized2(
                int* __restrict__ A,
                int* __restrict__ B,
                int* __restrict__ C,
                int M, int N, int K)
{
    // shared memory for cached partial data of A and B
    __shared__ int tile_A[BLOCK_SIZE_K][BLOCK_SIZE_M]; //[8,128], transpose for vectorized frag_a
    __shared__ int tile_B[BLOCK_SIZE_K][BLOCK_SIZE_N]; //[8,128]
    // register for C, replace before sum
    int accum[THREAD_SIZE_Y][THREAD_SIZE_X] = {0};
    ...
    for(int tile_idx = 0; tile_idx < K; tile_idx+=BLOCK_SIZE_K)
    {
        ...
        for(int i=0;i<BLOCK_SIZE_M;i+=A_TILE_ROW_STRIDE){
            ...
            FETCH_INT4(ldg_a_reg[ldg_index]) = FETCH_INT4(A[OFFSET(
                            A_TILE_ROW_START+i, //row
                            A_TILE_COL+tile_idx, //col
                            K)]);
            tile_A[A_TILE_COL+0][i+A_TILE_ROW_START] = ldg_a_reg[ldg_index];
            tile_A[A_TILE_COL+1][i+A_TILE_ROW_START] = ldg_a_reg[ldg_index+1];
            tile_A[A_TILE_COL+2][i+A_TILE_ROW_START] = ldg_a_reg[ldg_index+2];
            tile_A[A_TILE_COL+3][i+A_TILE_ROW_START] = ldg_a_reg[ldg_index+3];
        }
        // load B from global memory to shared memory
        #pragma unroll
        for(int i=0;i<BLOCK_SIZE_K;i+=B_TILE_ROW_STRIDE){
        ...
        for (int k_idx = 0; k_idx < BLOCK_SIZE_K; k_idx++) {
            ...
            #pragma unroll
            for(int thread_y = 0; thread_y < THREAD_SIZE_Y; thread_y++) {
                FETCH_INT4(frag_a[thread_y]) = FETCH_INT4(tile_A[k_idx][THREAD_SIZE_Y*ty + thread_y]);
            }
            // load B from shared memory to register: per row
            #pragma unroll
            for(int thread_x = 0; thread_x < THREAD_SIZE_X; thread_x+=4) {
